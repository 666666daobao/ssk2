...
# data_scaler_root_path = r"F:\Transformer\LTSF_Linear_main\LTSF_Linear_main\ONNX\Scaler"
data_scaler_path = r'ETTh1_scaler.csv'
# data_scaler_source_path = os.path.join(data_scaler_root_path, data_scaler_path)

# Scaler_root_path = r"F:\Transformer\LTSF_Linear_main\LTSF_Linear_main\ONNX\Scaler"
# scaler_path = r"ETTh1_scaler.pkl"
# scaler_source_path = os.path.join(Scaler_root_path, scaler_path)
...
scaler = train_scaler(data_path, features_columns)
...
def onnx_mertic(source_path,csv_path):
    data = pd.read_csv(source_path)
    new_predictions = pd.read_csv(csv_path)
    # 选择 ettmm1 数据的倒数 66610 行到最后一行
    # ettmm1_tail = ettmm1_data.iloc[66608:]
    etth1_tail = data.iloc[14348:]
    ...
    mse_total = 0
    mae_total = 0
    num_columns = len(etth1_tail.columns)

    for column in etth1_tail.columns[1:]:
        mse_column = mean_squared_error(etth1_tail[column], new_predictions[column])
        mae_column = mean_absolute_error(etth1_tail[column], new_predictions[column])
...
if __name__ == "__main__":
    ...
    # Call main function
    main_time= time.time()
    predictions = main(onnx_path, data_path, scaler,N=96)
    print("inference cost time: {}s".format(time.time() - main_time))
    features = 7
    ...
    # 加载ETTm1数据集以获取日期信息
    ettm1_df = pd.read_csv(data_path)
    ...
    onnx_mertic(data_scaler_source_path,'new_predictions_etth1.csv')
